...
int main() {
    ...
    while(!WindowShouldClose()) {
        ...
        if(IsKeyPressed(KEY_UP))    map[x][y][0] = !map[x][y][0];
        if(IsKeyPressed(KEY_LEFT))  map[x][y][1] = !map[x][y][1];
        if(IsKeyPressed(KEY_RIGHT)) map[x][y][2] = !map[x][y][2];
        if(IsKeyPressed(KEY_DOWN))  map[x][y][3] = !map[x][y][3];
        ...
        BeginDrawing();
            ...
            for(int i = 0; i < mW; i++) {
                for(int j = 0; j < mH; j++) {
                    DrawRectangleLines(
                        i * tileX, j * tileY, tileX, tileY, WHITE
                    );
